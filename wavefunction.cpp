...
eigen_t waves_s=initiate.find_boundstate(rmesh_p, estart,1,0,.5,tol);

eigen_t waves_d=initiate.find_boundstate(rmesh_p, estart,0,2,1.5,tol);
...
double S_s = initiate.sfactor( rmesh_p, waves_s.first , 0, 0.5, waves_s.second );
double S_d = initiate.sfactor( rmesh_p, waves_s.first , 2, 1.5, waves_s.second );
...
for(int ii=0;ii<rmesh_p.size();++ii){
      norm_s += rdelt* rmesh_p[ii] * waves_s.second[ii] ;
      norm_d += rdelt * std::pow(rmesh_p[ii] * waves_d.second[ii],2) ;
...
//            << " " << std::pow( asymp_d/normAsymp_d ,2)  *(.023309/0.000336918) <<" " <<std::pow( initiate.WaveArray[ii],2) <<std::endl;
      pott->setAM(0,.5);
      pott->setEnergy(10.);
      filee << rmesh[ii] << " " <<real(pott->nonlocalPart(rmesh[ii],rmesh[ii]))<< " "<<imag(pott->nonlocalPart(rmesh[ii],rmesh[ii]))
            << " " << 2.0 * waves_s.second[ii]/norm_s * S_s 
	    << " "<< 4. * std::pow( waves_d.second[ii]/norm_d ,2) * S_d  
            << " " << 2. * std::pow( asymp_s/normAsymp_s ,2) 
            << " " <<4. * std::pow( asymp_d/normAsymp_d ,2) 
            << " " <<2. * std::pow( initiate.WaveArray[ii]/normbob,2) 
	    <<std::endl;
}
*/
