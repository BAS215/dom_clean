...
///////**************************************************************************//////////////////
///////**************************************************************************//////////////////
vector<double> dom_nonlocal_r::dom_r_space_wavefunction(vector<double> rmesh )
 {
...
//   NuclearParameters Nu_n = read_nucleus_parameters( n_filename );
   ...
   // Construct Parameters Object
    ...
    boundRspace initiate(rmax,rsize, Nu.Ef, 0, lmax, Nu.Z, Zp, Nu.A, U1);
    eigen_t waves_function =initiate.find_boundstate(rmesh,Nu.Ef,n,l,J,tolerance);


  return waves_function.second;
 }
///////**************************************************************************//////////////////
///////**************************************************************************//////////////////
///////**************************************************************************//////////////////
...
//    string input_dir = "Input/";
    ...
    ////////
   // string parameters_filename = "Input.inp";
    string parameters_filename = "Input19Feb2015.inp";
...
// MatrixXcd dom_nonlocal_r::dom_r_space()
 ...
 // cout<<rsize<<endl;
   ...
   double rmax = rmesh[rsize-1];
    ...
    //string parameters_filename = "Input24Jan2015.inp";
    std::string parameters_filename ="Input19Feb2015.inp";
...
//  NuclearParameters Nu_n = read_nucleus_parameters( n_filename );
   ...
   // Construct Parameters Object
 //   Parameters p = get_parameters( parameters_filename, Nu.A, Nu.Z, Zp );
    Parameters p = get_parameters( parameters_filename, 40, 20, Zp );
   // cout<<Zp<<"\t"<<Nu.A<<"\t"<<Nu.Z<<"\t"<<endl;
  //  Parameters p = get_parameters( parameters_filename, 40, 20, Zp );
  ...
  for( int i = 0; i < rsize; ++i ) 
    ...
    U1-> setAM(l,J);
      ...
      // Vdom(i,i) +=  U1->localPart(rmesh[i]); 
       ...
       Vdomlocal[i]=U1->localPart(rmesh[i]); 
      //  cout<<Energy_cm<<"\t"<<rmesh[i]<<"\t"<<Vdomlocal[i]<<"\t"<<Vdom(i,i)<<endl;
     }  
  // Trying the bound state, it might need E<0 ???
...
//   string pot_local_file_r = "diffCrossXn/testE65/NEW/pot_local_file_rNSC.out";
 //  std::ofstream potlocalfile_r( pot_local_file_r.c_str() );
//   for( jr =0; jr<rsize; jr++)
  //    {
      // potlocalfile_r<<rmesh[jr]<<"\t"<<real( Vdomlocal_r[jr])<<endl;
      // cout<<rmesh[jr]<<"\t"<<real( Vdomlocal_r[jr])<<endl;
    //  }

 //  string pot_local_file = "diffCrossXn/testE65/NEW/pot_local_fileNSC.out";
 //  std::ofstream potlocalfile( pot_local_file.c_str() );
  // cout<< l << "\t"<<J <<"it is in k loop"<<endl;  
   // Transforming Irreducible Self-Energy
   for(ik=0; ik<ksize; ik++)
     {
       for(jk=0; jk<ksize; jk++)
	  {
	    ...
	    Vdom_kk(ik,jk) = sumOutside + sumlocal;
     //  if (ik > 0 && jk >0)     potlocalfile <<real(Vdom_kk(ik,jk))<<"\t";
           // potlocalfile <<k[ik]<<"\t"<<k[jk]<<"\t"<<real(Vdom_kk(ik,ik))<<"\t"<<imag(Vdom_kk(ik,ik))<<endl;
                                        //  <<"\t"<<real(sumlocal)<<"\t"<<imag(sumlocal)
                                        //  <<"\t"<<real(sumOutside)<<"\t"<<imag(sumOutside)<<endl;
	
          }
      //  if (ik > 0 ) potlocalfile<<endl;
     }
