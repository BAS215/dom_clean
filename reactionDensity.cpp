...
// Calculating the mid point of charge distribution and Particle number as well as R_rms from Point_charge distribution function//
	...
	{      
		...
		if (std::abs(rmesh[i]-middlethree1)<minnthree) 
			{ three1_index=i; 
    ...
    double ch_den_four1_chisq = std::pow( ( chdf[four1_index] - 0.0153698 )
                        / ( .01 * 0.000153698), 2 );
    
    double ch_den_five_chisq = std::pow( ( chdf[five_index] - 0.00658348 )
                        / ( .01 * 0.0000658348), 2 );
    

    double ch_den_five1_chisq = std::pow( ( chdf[five1_index] - 0.00274881 )
                        / ( .01 * 0.0000274881), 2 );
 ...
 //   double BoundChisquared = ch_den_chisq + R_rms_chisq + particle_chisq  + Energy_chisq; //+ tot_chisq_p_250 + tot_chisq_p_650;
   ...
   double BoundChisquared = ch_den_three_chisq + ch_den_three1_chisq + ch_den_four1_chisq + ch_den_four_chisq + ch_den_five_chisq + ch_den_five1_chisq ; 
   
    cout << "charge density at 0 =  " << chdf[0] << " " 
