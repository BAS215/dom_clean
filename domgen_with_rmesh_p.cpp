...
int main() {
   ...
   // std::string parameters_filename = parameters_string + ".inp";
    ...
    std::string output_dir = "Output_hosfit/Output_domgen/final/";
...
//			    else {	 
                            ...
                            d_mtx_p( i, j ) -=  Edelt * imag( G_p( i, j ) ) / M_PI
                                           / ( rmesh_p[i] * rmesh_p[j] * rdelt *(40./41.));
//			    }	
//                            if (m==10 && i == 2&& j == 3  ){cout <<E << " " << G(i,j) << std::endl;}
		        ...
		        if  ((l==0) && (xj==.5) && (N==1)) {
		       	         ...
		       	         wwavefunction <<rmesh[ii]<<" "<<S*(2.*xj+1)*QPF[ii]*QPF[ii]<<std::endl;
		                 summ_s += rdelt * std::pow(rmesh[ii] * QPF[ii],2);			   		
		            }
		        }
                        std::cout << "norm = "<< summ_s<<std::endl;
		        if  ((l==2) && (xj==1.5) && (N==0)) {
                            for (int ii=0 ; ii<rmesh.size(); ++ii) { 
                                wwwavefunction <<rmesh[ii]<<" "<<S*(2.*xj+1)*QPF[ii]*QPF[ii]<<std::endl;
                            }
                        }         
                    }*/
...
////////////////////// //////////// //////////// //////////// //////////// //////
        ...
        std::vector<double> chdf = 
            folded_ch_density( rmesh, rweights, point_dist_p, tz, Nu.A );
...
//std::cout<<i<<std::endl;
        chd_file << rmesh[i] << " " 
	         << chd_tot[i] * norm_fac << " " 
	         << rmesh[i]*rmesh[i]*chd_tot[i] * norm_fac << " "<<std::endl; 
