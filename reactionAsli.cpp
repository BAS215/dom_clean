...
// Calculating the mid point of charge distribution and Particle number as well as R_rms from Point_charge distribution function//
    ...
    double middle=2.0;
    double middleone=1.0;
    double middlethree=3.0;
    double middlefour=4.0;	
    double middleonehalf=1.5;
    int middle_index=0;
    int one_index=0;
    int three_index=0;
    int four_index=0;
	...
	{      
		...
		if (std::abs(rmesh[i]-middle)<minnthree) 
			{ three_index=i; 
    ...
    double ch_den_chisq = std::pow( ( chdf[0] - Chi_Squared_data.ch_den_0_exp )
                        / ( .1 * Chi_Squared_data.err_0_exp), 2 );

    double ch_den_one_chisq = std::pow( ( chdf[one_index] - 0.0825104 )
                        / ( .01 * 0.000825104), 2 );

    double ch_den_three_chisq = std::pow( ( chdf[three_index] - 0.0608052 )
                        / ( .05 * 0.000608052), 2 );

    double ch_den_four_chisq = std::pow( ( chdf[four_index] - 0.0278664 )
                        / ( .01 * 0.000278664), 2 );
    
    
    double ch_den_onehalf_chisq = std::pow( ( chdf[onehalf_index] - 0.0796949 )
                        / ( .1 * 0.000796949), 2 );
    
    double ch_den_middle_chisq = std::pow( ( chdf[middle_index] 
                               - Chi_Squared_data.ch_den_middle_exp ) 
                               /(.1 * Chi_Squared_data.err_middle_exp), 2 );
 ...
 //   double BoundChisquared = ch_den_chisq + R_rms_chisq + particle_chisq  + Energy_chisq; //+ tot_chisq_p_250 + tot_chisq_p_650;
   ...
   double BoundChisquared = ch_den_four_chisq + ch_den_chisq + ch_den_one_chisq; 
   
    cout << " ch at 4 = "<< ch_den_four_chisq << " ch at zero is = "
         <<  ch_den_chisq << "and at 1 is =" << ch_den_one_chisq<<endl; 
    cout << Nlevel <<  " " << "Energy chisq = " << Energy_chisq << endl;
    cout << " " <<endl;    
    cout << "charge density =  " << chdf[0] << " " 
         << "ch_den_chisq = " << ch_den_chisq << endl;

    cout << " " <<endl;    
    cout << " charge density at 1.5 = " << chdf[onehalf_index] << " " 
         << "ch_den_middle_chisq = " << ch_den_onehalf_chisq <<endl;
