...
//    std::string output_dir = "Output_" + parameters_string + "/Spectral_functions_kE/New/";
    std::string output_dir = "Output_hosfit/Spectral_functions_kE/New/";
...
//
        std::string nout = output_dir + "n_out.out";  
        std::ofstream n_out( nout.c_str() );
		...
		sum += S_of_kE_mtx2( i, j ) / fac;
            }
	    n_out<<kmesh2[i] << " " << sum << std::endl;					
            file4.close();
            file4.clear();
        }
//    } // end loop over tz
